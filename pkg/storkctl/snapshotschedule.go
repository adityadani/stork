...
var snapshotScheduleColumns = []string{"NAME", "PVC", "POLICYNAME", "PRE-EXEC-RULE", "POST-EXEC-RULE", "RECLAIM-POLICY", "ENABLED", "LAST-SUCCESS-TIME"}
...
func newCreateSnapshotScheduleCommand(cmdFactory Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	createSnapshotScheduleCommand.Flags().BoolVar(&enabled, "enabled", true, "Flag to denote whether schedule should be enabled on creation")
